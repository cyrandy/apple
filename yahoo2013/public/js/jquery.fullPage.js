...
(function($) {
	$.fn.fullpage = function(options) {
		...
		$('.steps').wrapInner('<div id="superContainer" />');

		//creating the navigation dots 
		if (options.navigation) {
			$('.steps').append('<div id="fullPage-nav"><ul></ul></div>');
		...
		*/
		...
		function resizeMe(displayHeight, displayWidth) {
			//Standard height, for which the .steps font size is correct
			...
			if (displayHeight < 825 || displayWidth < 900) {
				...
				$(".steps").css("font-size", newFontSize + '%');
			} else {
				$(".steps").css("font-size", '100%');
			}
		}
